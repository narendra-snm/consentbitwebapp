...
class TCFManager {
  constructor() {
    ...
    this.config = {
      ...
      cmpVersion: 1,
      consentScreen: 1,
      // ▼ CHANGE THIS to test a language on page load: one of SUPPORTED_LANGUAGES.
      // At runtime use window.tcfManager.setLanguage('de') instead — no reload.
      language: 'en',
  ...
  async applyLanguage(language) {
    ...
    try {
      ...
      // Keep the English GVL rather than leaving the banner with no text at all.
