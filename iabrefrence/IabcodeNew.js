...
function applyStaticStrings() {
  // The markup already ships the English copy, so on an English banner there is
  // nothing to replace. Skipping keeps the DOM byte-identical to the pre-i18n
  // build for every existing single-language site — no nodes destroyed and
  // recreated, no listeners at risk. Once a non-English language has been
  // applied the flag stays set, so switching back to English still restores it.
