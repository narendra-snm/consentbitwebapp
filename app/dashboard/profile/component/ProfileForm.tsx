      ...
      <div className="flex flex-col gap-6 w-[376px] text-left">
        ...
        <div>
          ...
          <input
            ...
            placeholder={displayEmail !== "—" ? displayEmail : "Billing email address"}
          ...
          />
          {billingEmailError && (
            <p className="text-red-500 text-xs mt-1">{billingEmailError}</p>
          )}
          {billingEmailSuccess && (
            <p className="text-green-600 text-xs mt-1">Billing email saved.</p>
          )}
          <button
            type="button"
            disabled={billingEmailSaving}
            onClick={() => onSaveBillingEmail?.(billingEmailInput)}
            className="mt-3 px-4 py-2 bg-[#6366F1] text-white text-sm rounded-md hover:bg-[#4F46E5] disabled:opacity-50 transition-colors"
          >
