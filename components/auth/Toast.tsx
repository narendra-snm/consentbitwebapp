...
export default function Toast({
  ...
  // 'animating' drives the CSS class: true → slide-in, false → slide-out
  const [animating, setAnimating] = useState(false);

  useEffect(() => {
    if (isVisible) {
      setMounted(true);
      // Let the browser paint the element first, THEN trigger enter animation
      requestAnimationFrame(() => {
        requestAnimationFrame(() => setAnimating(true));
      });
    } else {
      // Trigger exit animation, then unmount after it completes (300 ms)
      setAnimating(false);
  ...
  return (
    <>
      {/* Keyframes injected once — safe to repeat, browsers deduplicate */}
      <style>{`
        @keyframes toast-in {
        ...
        .toast-enter {
          animation: toast-in  0.3s cubic-bezier(0.42, 0, 0.58, 1) forwards;
        }
        .toast-exit {
          animation: toast-out 0.3s cubic-bezier(0.42, 0, 0.58, 1) forwards;
        }
      `}</style>

      <div
        className={`fixed left-1/2 top-[20px] z-50 ${animating ? "toast-enter" : "toast-exit"}`}
        style={{
          width: "834px",
          height: "83px",
          // base transform keeps it centred; animation overrides it per keyframe
      ...
      >
        <div className="relative size-full">
              ...
              {isError ? (
                <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 45.7138 45.9638">
                  <g id="Group 10123394">
                    ...
                    <path d="M18 26.5611L26.4871 18.6632" stroke="var(--stroke-0, #AC2734)" strokeLinecap="round" strokeWidth="2" />
                    <path d={svgPaths.p3f52ec80} stroke="var(--stroke-0, #AC2734)" strokeLinecap="round" strokeWidth="2" />
                  </g>
                  <defs>
          ...
          {/* Message text */}
          <p
            className="absolute font-['DM_Sans:SemiBold',sans-serif] font-semibold leading-[20px] left-[72px] text-[18px] text-white top-[25px] whitespace-nowrap"
