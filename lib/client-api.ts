 ...
 * Password login.
 *
 * The password travels as plaintext over HTTPS and is hashed server-side with PBKDF2.
 * TLS protects it in transit; the salted hash protects it at rest.
 * It is deliberately NOT hashed in the browser: the previous client-side SHA-256 scheme
 * made the stored value directly replayable — anyone who could read the database could
 * authenticate as any user.
 * The worker still accepts an RSA-encrypted `passwordEnc` field; this client no longer
 * uses it, since it bought no real protection over TLS and cost a public-key fetch on
 * every page load.
 */
...
/**
 * Signup WITHOUT a verification code — creates the account in one request and returns
 * with the session cookie already set, so the caller can go straight to /dashboard.
 *
 * Uses the same encrypted-field + stale-key-retry path as login(). The plaintext never
 * leaves the browser when a key is available; the worker hashes with PBKDF2 either way.
 *
 * TRADE-OFF vs signup(): nothing here proves the address belongs to the person
 * registering it. A confirmation link is emailed afterwards, and paid checkout stays
 * blocked until it is clicked.
 */
